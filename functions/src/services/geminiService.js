...
class GeminiService {
    ...
    static CACHE_TTL = 10 * 60 * 1000; // 10 minutes
    static async getArtieMetadata() {
    ...
    static async generateSystemPrompt(userId, userRole, venueId) {
        return `
ROLE & PERSONA
...
`;
    ...
    async generateArtieResponse(model, contents, temperature = 0.7, systemInstruction, tools, cachedContent) {
    ...
    async generateArtieResponseStream(model, contents, temperature = 0.7, systemInstruction, tools, cachedContent) {
    ...
    async analyzeEvent(event) {
        ...
        if (!text)
            throw new Error("Artie failed to analyze event.");
    ...
    async getTriage(question) {
        return "I'm ready to serve, boss.";
    }
    async generateManagerSuggestion(stats, venue) {
        ...
        Amenities: ${venue.amenityDetails?.map((a) => a.name).join(', ')}
    ...
    - Pitch it as a partner in the business (concise, pragmatic).
        ...
        let text = response.candidates?.[0]?.content?.parts?.[0]?.text?.trim();
        if (!text)
            throw new Error("Artie failed to generate suggestion.");
        text = text.replace(/```json\n?|```/g, '').trim();
        try {
            return JSON.parse(text);
        }
