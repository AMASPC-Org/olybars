...
export class ArtieLLM {
    /**
     * Generates a professional press release for an event.
     * Bridged to the OlyBars AI Backend.
     */
    static async generatePressRelease(venue: Venue, event: AppEvent): Promise<string> {
        try {
            const response = await fetch(`${API_BASE_URL}/ai/generate-press-release`, {
                method: 'POST',
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || 'Artie failed to draft the press release.');
            }

            const data = await response.json();
            return data.pressRelease;
        } catch (error) {
            console.error('ArtieLLM Error:', error);
