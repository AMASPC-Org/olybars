...
export const AmenityCard: React.FC<AmenityCardProps> = ({ venue, amenity, onCheckIn }) => {
    ...
    return (
        <div className="bg-[#1e293b]/50 backdrop-blur-md rounded-2xl border border-white/10 p-4 hover:border-primary/30 transition-all group mb-4">
            <div className="flex justify-between items-start mb-3">
                <div>
                    ...
                    <div className="flex items-center gap-2">
                        <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded flex items-center gap-1 ${isBuzzing ? 'bg-orange-500/20 text-orange-400' : 'bg-blue-500/20 text-blue-400'
                            }`}>
                            {isBuzzing ? <Flame size={10} /> : <Snowflake size={10} />}
                            {venue.status.toUpperCase()}
                        </span>
                        {amenity.isLeaguePartner && (
                ...
                <div className="text-right">
                    <span className="text-2xl font-black text-white leading-none font-league">
                        {amenity.available !== undefined ? `${amenity.available}/${amenity.count}` : amenity.count}
                    </span>
                    <p className="text-[8px] text-slate-500 font-bold uppercase">{amenity.available !== undefined ? 'Available' : 'Units'}</p>
                </div>
            </div>

            <button
                onClick={() => onCheckIn(venue.id, amenity.id)}
            ...
            >
                <span className="text-[10px] uppercase tracking-widest font-league">Check-In to this {amenity.name}</span>
